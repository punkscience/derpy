...
func TestUploadDownloadChunk(t *testing.T) {
	...
	// Upload
	if err := uploadChunk(context.Background(), srv.URL, encrypted, sum, privKey); err != nil {
...
func TestUploadChunkToServers_PartialFailure(t *testing.T) {
	...
	accepted, err := uploadChunkToServers(context.Background(), servers, enc, sum, privKey)
	if err != nil {
